...
Feature: Check shippings tent and MO
    ...
    Scenario Outline: 07_ChangeMaterialTentRefInShippings
        ...
        And I click on this button "Кнопка Сохранить"
        And I expect this element "Loader" is not visible
        # проверка что тип кузова сменился на Реф
        And thes element "История в накладной" contains the text "Значение поля Тип кузова изменено на 'Реф'"
        # Удаление артикулов Реф
        And I delete material "<materialRef>"
        And I click on this button "Кнопка Сохранить"
        # проверка что тип кузова сменился на Тент
        And thes element "История в накладной" contains the text "Значение поля Тип кузова изменено на 'Тент'"
        ...
        Examples:
            | login coordinator           | shipping warehouse | delivery warehouse | countPallet | shipping date | delivery date |materialTent|materialRef|
            | Roust Transport Coordinator | РУСТ ИНК Москва          | Котовский         | 4           | 1             | 7             |   105543    |   303308   |
