...
Feature: Create order without min fields
    create order without requirement field client order number
    and check it's status   

    Background:
        Given I log in as a user Administrator on homepage

    Scenario Outline: Attempt to create order without client order number field and check that attempt failed
        Then I expects that element "order_without_client_order_number" does not exist
        ...
        Examples:
            ...
            | order_value_without_client_order_number |                        | order_date | twenty_date | astrahan_sold_to_value |
