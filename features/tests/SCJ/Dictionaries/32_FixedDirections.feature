...
Feature:
    ...
    Scenario Outline: 32_FixedDirections
       ...
       #And I set filter checkbox "<delivery warehouse>" in field "Склад доставки"
        ...
        And thes element "Грид Справочников" contains the text "<delivery region>"
        # Редактирование записи
        When I click on this in div "<delivery region>"
        ...
        And I expect this element "Loader" is not visible
        Then thes element "Грид Справочников" contains the text "<delivery region>"
        #Делаем запись не активным
        And I click on this in div "<delivery region>"
        And I click on this in checkbox "Нет"
        And I click on this in button "Сохранить"
        And I expect this element "Loader" is not visible

        Examples:
            | login coordinator         | TK        | shipping city | delivery city | shipping warehouse | ship to code | shipping region | delivery region  | type tc  | quote | quote2 |
            | SCJ Transport Coordinator | Русглобал | г. Москва     | Новосибирск     | Томилино           | 120964 (АО ДМС Новосибирск) | обл. Московская  | обл. Новосибирская | Тент 20т | 50    | 90     |
